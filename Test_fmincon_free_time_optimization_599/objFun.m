function [J,Jacobian_J] = objFun(z,sysParam)
    ...
    if isTargetConstraint == 0  % Target as a part of the cost function
        ...
        J = z(zDim,1).^2 + TargetPosWeight * norm(z([zDim-zDim_per_step+1+1,zDim-zDim_per_step+1+3],1)-TargetPos); % x and y position as objective
        Jacobian_J(zDim,1) = 2*z(zDim,1);
        Jacobian_J([zDim-zDim_per_step+1+1,zDim-zDim_per_step+1+3],1) = 2*TargetPosWeight.*(z([zDim-zDim_per_step+1+1,zDim-zDim_per_step+1+3],1)-TargetPos); 
